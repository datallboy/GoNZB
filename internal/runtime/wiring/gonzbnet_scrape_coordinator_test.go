...
func TestGoNZBNetScrapeCoordinatorPublishesClaimAndComplete(t *testing.T) {
	...
	if len(store.events) != 4 {
		t.Fatalf("expected claim, observation, checkpoint, and complete events, got %d", len(store.events))
	}
	if store.events[0].EventType != coverage.TypeRangeClaim || store.events[1].EventType != coverage.TypeGroupObservation || store.events[2].EventType != coverage.TypeCoverageCheckpoint || store.events[3].EventType != coverage.TypeRangeComplete {
		t.Fatalf("unexpected event types: %s, %s, %s, %s", store.events[0].EventType, store.events[1].EventType, store.events[2].EventType, store.events[3].EventType)
	}
	...
	if store.projected != 4 {
		t.Fatalf("expected four projected events, got %d", store.projected)
	}
}
