...
var indexerMaintenanceTaskDefinitions = []maintenanceTaskDefinition{
	...
	{TaskKey: "partition_retention_drop", Label: "Partition Retention Drop", Purpose: "Reports whether source/work tables are ready for daily native partition drops and blocks destructive drops on non-partitioned schemas.", Risk: "high", SpaceEffect: "No OS space is reclaimed until source/work tables are rebuilt as native daily partitions.", SupervisorEffect: "Report-only on the current non-partitioned schema; future partition drop must verify no active work remains for the day.", DataEffect: "Dry-run reports partition readiness. Run mode intentionally performs no drops until native partitions exist.", ReleaseSafety: "Durable release/catalog/archive tables are outside the source/work partition target set.", Destructive: true, UsesBatchSize: true, MinIntervalHours: 24, Warnings: []string{"fresh/rebuild-first partition conversion is still required before this can reclaim space", "batch size is interpreted as retention-days horizon for the readiness report"}},
