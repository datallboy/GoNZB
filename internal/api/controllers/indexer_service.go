...
func (s *runtimeIndexerService) executeMaintenanceTask(ctx context.Context, taskKey string, dryRun bool) (*pgindex.MaintenanceTaskResult, error) {
	...
	case "outcome_reconcile":
		if dryRun {
			return &pgindex.MaintenanceTaskResult{TaskKey: taskKey, DryRun: true, EstimatedRowsByTable: map[string]int64{}, Warnings: []string{"run reconciliation to refresh outcome metadata; reconciliation never deletes source data"}}, nil
		}
		return store.ReconcileSourceBucketOutcomes(ctx, cfg.BatchSize, sourceBucketOutcomePolicyFromRuntime(runtime))
	...
	case "partition_retention_drop":
		...
		if runtime.Indexing == nil || !runtime.Indexing.Retention.ExecuteOutcomePurge {
			return nil, fmt.Errorf("destructive outcome purge is disabled; enable indexing.retention.execute_outcome_purge after reviewing the dry-run")
		}
...
var indexerMaintenanceTaskDefinitions = []maintenanceTaskDefinition{
	...
	{TaskKey: "outcome_reconcile", Label: "Outcome Reconciliation", Purpose: "Classifies provider/group/source-day work as active, durably successful, or bounded no-yield from ingestion and downstream progress.", Risk: "low", SpaceEffect: "No storage reclaim; updates the source-bucket outcome ledger.", SupervisorEffect: "Supplies the terminal-state gate used by partition retention.", DataEffect: "Updates outcome metadata only and never deletes source rows.", ReleaseSafety: "Success requires a durable archived NZB plus durable catalog files; active work remains non-terminal.", Destructive: false, UsesBatchSize: true, MinIntervalHours: 1},
	...
	{TaskKey: "partition_retention_drop", Label: "Outcome Partition Retention", Purpose: "Drops native daily source/work partitions only when every represented provider/group bucket has a terminal outcome.", Risk: "high", SpaceEffect: "Drops whole partition tables, returning those table files to PostgreSQL immediately without row-by-row deletes.", SupervisorEffect: "Removes terminal source/header/yEnc/binary-part work; stages can no longer rebuild it.", DataEffect: "Drops only terminal dated source/work/projection partitions; durable release catalog, archive metadata, and NZBs remain.", ReleaseSafety: "Requires outcome reconciliation, zero open work, archive+catalog success or bounded no-yield, and day-scoped default/blocker checks.", Destructive: true, UsesBatchSize: true, MinIntervalHours: 24, Warnings: []string{"high risk: dry-run first", "destructive execution is disabled by default", "batch size is the maximum terminal source days per run"}},
	{TaskKey: "partition_default_rehome", Label: "Offline Partition Default Rehome", Purpose: "Moves quarantined default-partition rows into their exact dated native children.", Risk: "medium", SpaceEffect: "No immediate storage reclaim; restores partition pruning and terminal retention eligibility.", SupervisorEffect: "Requires the indexer supervisor and all writer stages to be stopped.", DataEffect: "Copies default rows into dated children and removes the moved defaults without deleting logical data.", ReleaseSafety: "Preserves source, binary, yEnc, inspect, and release projection rows.", Destructive: true, UsesBatchSize: true, MinIntervalHours: 24, Warnings: []string{"offline only: stop all indexer writer stages", "dry-run first", "batch size is number of default-partition days to move, capped at 7"}},
...
func sourceBucketOutcomePolicyFromRuntime(runtime *app.RuntimeSettings) pgindex.SourceBucketOutcomePolicy {
	defaults := app.DefaultRuntimeSettings()
	retention := defaults.Indexing.Retention
	if runtime != nil && runtime.Indexing != nil {
		retention = runtime.Indexing.Retention
	}
	return pgindex.SourceBucketOutcomePolicy{
		SourceSettleHours:    retention.SourceSettleHours,
		NoYieldGraceDays:     retention.NoYieldGraceDays,
		YEncTerminalAttempts: retention.YEncTerminalAttempts,
	}
}
...
func applyStagePatch(dst *app.IndexingStageRuntimeSettings, patch indexerStageConfigPatch) {
	...
	if patch.BinaryPartUpsertDBChunkSize != nil {
		dst.BinaryPartUpsertDBChunkSize = *patch.BinaryPartUpsertDBChunkSize
	}
	if patch.BinaryStatsRefreshDBChunkSize != nil {
		dst.BinaryStatsRefreshDBChunkSize = *patch.BinaryStatsRefreshDBChunkSize
	}
	if patch.SubjectQueueBatchSize != nil {
		dst.SubjectQueueBatchSize = *patch.SubjectQueueBatchSize
	}
