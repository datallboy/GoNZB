...
func RegisterRoutes(e *echo.Echo, appCtx *app.Context) {
	...
	if modules.API.Enabled && modules.UsenetIndexer.Enabled {
		...
		v1AdminIndexer.PATCH("/maintenance/tasks/:task", indexerAdminCtrl.PatchMaintenanceTask, authMiddleware(authSvc, appCtx.Config.API.Key, false, auth.PermissionIndexerRuntimeConfigure))
		v1AdminIndexer.POST("/maintenance/tasks/:task/dry-run", indexerAdminCtrl.DryRunMaintenanceTask, authMiddleware(authSvc, appCtx.Config.API.Key, false, auth.PermissionIndexerRuntimeRun))
		v1AdminIndexer.POST("/maintenance/tasks/:task/run", indexerAdminCtrl.RunMaintenanceTask, authMiddleware(authSvc, appCtx.Config.API.Key, false, auth.PermissionIndexerRuntimeRun))
