...
func (s *Service) RunOnce(ctx context.Context, limit int) (Result, error) {
	...
	staleClaims, err := s.store.ListStaleCoverageRangeClaims(ctx, s.poolID, limit)
	if err != nil {
		return result, err
	}
...
func selectReplacementNode(claim pgindex.CoverageClaimRecord, nodes []pgindex.CoverageScannerNode) string {
	...
	workID := fmt.Sprintf("%s:%s:%d-%d", strings.TrimSpace(claim.PoolID), strings.TrimSpace(claim.Group), claim.RangeStart, claim.RangeEnd)
