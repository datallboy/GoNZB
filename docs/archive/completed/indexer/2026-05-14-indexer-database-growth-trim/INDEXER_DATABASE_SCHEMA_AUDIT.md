...
This doc was updated on the same sprint branch as the implementation work. Treat the commit order in `docs/archive/completed/indexer/2026-05-14-indexer-database-growth-trim/INDEXER_DATABASE_GROWTH_TRIM_PLAN.md` as the execution order used for audit and cleanup sessions during that sprint.

Use `docs/INDEXER_CURRENT_SCHEMA_AND_SYSTEM_INTERACTIONS.md` when a cleanup decision depends on whole-system ownership rather than only column-by-column usage.
