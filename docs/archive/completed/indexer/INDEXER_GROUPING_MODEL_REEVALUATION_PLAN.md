...
- assemble is working, but the indexed groups are much larger than the earlier baseline
- the high-volume groups have far more pending headers than assembled headers, so release formation is starved by backlog depth
...
- `alt.binaries.example.high-volume`: `22.1M` headers, `6.9M` assembled, `15.3M` pending
- `alt.binaries.example.medium-volume`: `10.0M` headers, `3.1M` assembled, `6.9M` pending
- `alt.binaries.example.baseline`: `5.0M` headers, `0.96M` assembled, `4.1M` pending
...
Investigated an anonymized PAR2 file after a manual read showed target files
under a different synthetic archive stem.

Current DB state:

- an anonymized binary exists in `alt.binaries.example.high-volume`
- the PAR2 file is assembled as a one-part contextual binary
- an anonymized release already formed from that single tiny PAR2-like binary before the stronger gates were added
- no binaries currently exist for the target archive stem
...
- another nearby synthetic PAR2 stem is assembled as fragment-only with
  `10/29` files and `0%` complete main payload coverage
- a nearby anonymized archive family is assembled with `17/35` binaries and
  `42.86%` expected file coverage
