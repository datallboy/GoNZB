...
- `docs/archive/development/indexer/INDEXER_TEST_QUERIES.md` now includes reusable operator queries for pending assembly summary, why a header is still unassembled, why a family is still dirty but unformed, and whether a family is fragment-only or release-actionable
