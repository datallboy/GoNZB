...
Representative anonymized proof from `alt.binaries.example.high-volume` showed
thousands of unqueued opaque singletons in one minute. A synthetic
`articleprobe` example found:

```text
=ybegin part=12 total=400 line=128 size=123456789 name=SyntheticToken.part04.rar
=ypart begin=1000001 end=2000000
```
...
- the largest unqueued one-minute singleton cohorts observed were anonymized
  high-volume groups with several thousand singles each;
...
Synthetic regression fixture to add:

- `SyntheticObfuscatedToken.bin` in `alt.binaries.example`:
  - Subject supplies `[1/8]`, the stable obfuscated filename,
    `(12/400)`, and a synthetic size;
  - sample BODY supplies matching part, total, and size fields but a different
    randomized `name=` value;
  - the anonymized observation showed many distinct part numbers split across
