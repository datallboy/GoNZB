...
```text
[1/8] - "SyntheticObfuscatedToken.bin" yEnc (12/400) 123456789
```
...
Synthetic failure example:

- a stable obfuscated Subject token appears with consistent multipart fields;
- the source is a synthetic `alt.binaries.example` group;
- the observed subset is split into many singleton weak binaries because
  randomized poster context was included in the grouping key;
