...
## On Hold: Database Write-Contention Isolation Plan

Status: on hold until the current PAR2 batching, yEnc work-item, and remaining assemble follow-up items are complete.

Purpose:

- capture the longer-run database plan that surfaced from the serve-vs-once assemble analysis
- define how to reduce row-lock and summary-refresh contention before considering Redis, an external queue, or a separate worker topology
- keep this work visible because it overlaps with the remaining PAR2 and yEnc backlog, but do not let it replace those near-term items yet
...
- the issue is not NNTP capacity and is not primarily supervisor overhead; it is shared Postgres write pressure and lock overlap
- `inspect_par2` has already shown real deadlock behavior in logs, which confirms this is not just a theoretical concern

Shared hot surfaces to isolate:
...
This work should start only after the current active items are farther along:

1. finish PAR2 batch persistence and telemetry
2. finish the yEnc work-item design and exact-count path
3. complete the remaining assemble telemetry and summary-refresh review
4. then open a focused schema/write-contention sprint using the evidence from those three tracks

### Exit Criteria For A Future Sprint
