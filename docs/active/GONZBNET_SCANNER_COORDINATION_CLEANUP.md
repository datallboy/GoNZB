...
- Honor trusted, provider-scope-compatible remote active claims when
  `scanner_respect_remote_claims` is enabled.
- Honor trusted, provider-scope-compatible completed ranges for scanner-mode
...
- Do not suppress article-number ranges across unknown provider scopes.
